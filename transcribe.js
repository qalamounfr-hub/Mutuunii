...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!groqRes.ok) {
      const ra = groqRes.headers && groqRes.headers.get ? groqRes.headers.get("retry-after") : null;
      const retryAfterMs = ra ? Math.round(parseFloat(ra) * 1000) : null;
      return {
        statusCode: groqRes.status,
        body: JSON.stringify({ error: (data && data.error && data.error.message) || "Erreur du service Groq.", retryAfterMs }),
      };
    }
